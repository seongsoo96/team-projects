package ppeonfun.controller.user.maker.requirement;
...
import ppeonfun.service.user.maker.requirement.RequirementService;

@Controller("user.maker.RequirementController")
@RequestMapping(value="/user/maker/requirement")
public class RequirementController {
	...
	public String view(Project project, Model model) {
		logger.info("/user/maker/requirement/view");
		...
		if(project!=null && "N".equals(project.getpState())) {//프로젝트가 승인이 안 되었을때
			...
			if(isProject) {
				return "redirect:/user/maker/requirement/modify?pNo="+project.getpNo();
			}else {
				return "redirect:/user/maker/requirement/write?pNo="+project.getpNo();
			}	
		}
	...
	public void writeForm(Project project, Model model) {
		logger.info("/user/maker/requirement/write [GET]");
		String name = requirementService.selectByName(project);
	...
	public String modify(Requirement requirement, MultipartFile file, String storedName){
		logger.info("/user/maker/requirement/modify [POST]");
		logger.info("requirement {}", requirement);
		logger.info("file {}", file);
