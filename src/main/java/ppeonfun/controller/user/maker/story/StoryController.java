package ppeonfun.controller.user.maker.story;
...
import ppeonfun.service.user.maker.story.StoryService;

@Controller("user.maker.StoryController")
@RequestMapping("/user/maker/story")
public class StoryController {
	...
	public String view(Project project, Model model) {
		logger.info("/user/maker/story/view");
		...
		if(project!=null && "N".equals(project.getpState())) {//프로젝트가 승인이 안 되었을때
			...
			if(isProject) {
				return "redirect:/user/maker/story/modify?pNo="+project.getpNo();
			}else {
				return "redirect:/user/maker/story/write?pNo="+project.getpNo();
			}	
		}
	...
	public void writeForm(Project project, Model model) {
		logger.info("/user/maker/story/write [GET]");
		String name = storyService.selectByName(project);
	...
	public String write(Story story, MultipartHttpServletRequest mtfRequest) {
		logger.info("/user/maker/story/write [POST]");
		...
		return "redirect:/user/maker/project/view?pNo="+story.getpNo();
	}
	
	@RequestMapping(value="/modify", method=RequestMethod.GET)
	public void modifyForm(Project project, Model model ) {
		logger.info("/user/maker/story/modify [GET]");
	...
	public String modify(Story story,  MultipartHttpServletRequest mtfRequest){
		logger.info("/user/maker/story/modify [POST]");
		logger.info("story {}", story);
