package ppeonfun.controller.user.maker.reward;
...
import ppeonfun.service.user.maker.reward.RewardService;


@Controller("user.maker.RewardController")
@RequestMapping("/user/maker/reward")
public class RewardController {
	...
	public String view(Project project, Model model) {
		logger.info("/user/maker/reward/view");
		...
		if(project!=null && "N".equals(project.getpState())) {//프로젝트가 승인이 안 되었을때
			...
			return "redirect:/user/maker/reward/write?pNo="+project.getpNo();	
		}
	...
	public void writeForm(Project project, Model model) {
		logger.info("/user/maker/reward/write [GET]");
		String name = rewardService.selectByName(project);
	...
	public String write(Reward reward) {
		logger.info("/user/maker/reward/write [POST]");
		logger.info("reward {}", reward);
		
		rewardService.updateRewardState(reward);
		
		return "redirect:/user/maker/project/view?pNo="+reward.getpNo();
	}
	...
	public String ajaxWrite(Reward reward, Model model) {
		...
		return "/user/maker/reward/ajaxInput";
	}
