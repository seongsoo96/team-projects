package ppeonfun.controller.user.maker.information;
...
import ppeonfun.service.user.maker.information.InformationService;

@Controller("user.maker.InformationController")
@RequestMapping("/user/maker/information")
public class InformationController {
	...
	public String view(Project project, Model model) {
		logger.info("/user/maker/information/view");
		...
		if(project!=null && "N".equals(project.getpState())) {//프로젝트가 승인이 안 되었을때
			...
			if(isProject) {
				return "redirect:/user/maker/information/modify?pNo="+project.getpNo();
			}else {
				return "redirect:/user/maker/information/write?pNo="+project.getpNo();
			}	
		}
	...
	public void writeForm(Project project, Model model) {
		logger.info("/user/maker/information/write [GET]");
		String name = informationService.selectByName(project);
	...
	public String write(Information information, MultipartFile file, @RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd") Date endDate) {
		logger.info("/user/maker/information/write [POST]");
		...
		return "redirect:/user/maker/project/view?pNo="+information.getpNo();
	}
	
	@RequestMapping(value="/modify", method=RequestMethod.GET)
	public void modifyForm(Project project, Model model ) {
		logger.info("/user/maker/information/modify [GET]");
	...
	public String modify(Information information, MultipartFile file, String storedName,  @RequestParam @DateTimeFormat(pattern = "yyyy-MM-dd") Date endDate){
		logger.info("/user/maker/information/modify [POST]");
		logger.info("information {}", information);
		logger.info("file {}", file);
